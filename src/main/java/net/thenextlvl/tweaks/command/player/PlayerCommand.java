...
abstract class PlayerCommand {
    ...
    PlayerCommand(final TweaksPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public LiteralCommandNode<CommandSourceStack> create(final String name, final String permission, final String permissionOther) {
        return Commands.literal(name)
                ...
                .then(Commands.argument("player", ArgumentTypes.player())
                        ...
                        .executes(context -> {
                            final var players = context.getArgument("player", PlayerSelectorArgumentResolver.class);
    ...
    protected int execute(final CommandSender sender) {
        if (sender instanceof final Player player) return execute(sender, player);
