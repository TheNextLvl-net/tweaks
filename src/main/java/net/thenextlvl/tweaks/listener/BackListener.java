...
public final class BackListener implements Listener {
    ...
    public BackListener(final TweaksPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    private boolean isSafe(final Block block) {
        ...
        for (final var face : faces) if (isDangerousIn(block.getRelative(face))) return false;
        return true;
    }

    private boolean isDangerousOn(final Block block) {
    ...
    private boolean isDangerousIn(final Block block) {
    ...
    public void onTeleport(final PlayerTeleportEvent event) {
        ...
        final var location = isSafe(event.getFrom().getBlock()) ? event.getFrom()
    ...
    public void onDeath(final PlayerDeathEvent event) {
        ...
        final var last = isSafe(location.getBlock()) ? location : plugin.backController().getLastSafeLocation(player);
